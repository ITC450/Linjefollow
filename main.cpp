...
void vej_foelger(Mat&cameraFrame, int rows, int cols, int slice, int&afvigelse)
{
   //int afvigelse;
   //Mats and containers
   Mat cvt;
...
void motor_kontrol_enhed(vector<int>&ids, Mat&cameraFrame, int rows, int cols, int point, int&status, int&frames)
{
   ...
   while (quit)
   {
      {
        unique_lock<mutex>lky(y);
        cv2.wait(lky, []{return ready == false;});
        if (ids[0] >= 0)
        {
...
void NN(Mat&cameraFrame, vector <int> &id, int&status)
{
   id.push_back(-1);
   id.push_back(0);
